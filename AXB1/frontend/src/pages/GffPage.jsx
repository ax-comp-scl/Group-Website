...
export default function GFFPage(){
    ...
    const [cpu, setCpu] = useState(formData.gff.cpu)

    const validateGFFFile = (file) => {
        const regex = /\.(gff|gtf|gff3)$/i
        return regex.test(file.name) ? null : {
            code: "file-invalid-type",
            message: "Tipo de arquivo inválido. Somente arquivos .gff, .gtf, ou .gff3 são permitidos."
        }
    }
    ...
    return(
        <>
            <div className="flex flex-col gap-10 items-center">
                <Dropzone validator={validateGFFFile} label="Arquivo GFF" textOnHover="GFF3 genome file indexed with tabix(see http://www.htslib.org/doc/tabix.html)"/>
                <AccordionComponent
                itens ={[
                    {
                        isRequired: true,
                    ...
                    {
                        fields: [
                            <SelectComponent options={doiOptions} defaultSelectedKeys={doi} label="doi" setValue={setDoi} textOnHover="DOI of the article reference to this sequence. E.g.: 10.1111/s12122-012-1313-4"/>,
                            <InputComponent label="Ignore" type="text" value={ignore} onValueChange={setIgnore} textOnHover="List of feature types to ignore (eg. chromosome,scaffold)"/>,
                            <InputComponent type="number" label="cpu" placeholder="0" value={cpu} onValueChange={setCpu} textOnHover="Number of threads"/>,
                            <CheckboxComponent name="qtl" isSelected={qtl} onValueChange={setQtl} textOnHover="Set this flag to handle GFF files from QTLDB"/>,
                        ]
                    },
                ]}/>
